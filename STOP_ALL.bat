...
echo.
REM Kill any remaining Python processes related to RealEstateMaster using modern PowerShell (fast)
echo Cleaning up background Python workers...
powershell -Command "Get-Process python -ErrorAction SilentlyContinue | Where-Object { $_.CommandLine -like '*RealEstateMaster*' } | Stop-Process -Force" 2>nul
