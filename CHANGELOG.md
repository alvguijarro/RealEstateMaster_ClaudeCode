...
- **Optimización de CAPTCHA Solvers**: Se ha invertido el orden de resolución en `solve_captcha_advanced` para intentar siempre el **Slider Local** antes que llamar a **2Captcha** (incluso para DataDome). Esto optimiza costes y reduce el tiempo de resolución ya que el solver local es altamente efectivo.
- **Robustez ante errores de red (Peer Connection)**: Implementado un "cooldown" de 5 segundos tras detectar el error `Failed sending data to the peer` en Playwright. Esto estabiliza los reintentos de navegación cuando el proceso del navegador o la red sufren micro-cortes.
- **Mejora de diagnóstico 2Captcha**: Añadido registro detallado del tipo de excepción al fallar el solver de coordenadas para facilitar la monitorización de cuotas o errores de API.
- **Fix botón "Detener" no detenía el scraping**: Corregido un problema crítico donde pulsar "Detener" durante el scraping no detenía el proceso inmediatamente. La causa raíz era que la señal de parada (`asyncio.Event`) solo se procesaba cuando el event loop de asyncio liberaba el control, lo cual podía tardar varios segundos si Playwright estaba navegando. Se ha añadido un `threading.Event` como señal paralela instantánea y una propiedad unificada `_should_stop` que comprueba ambos mecanismos. Además, el bloque `except Exception` genérico en el bucle de propiedades ahora verifica la señal de parada antes de continuar al siguiente elemento.
- **Fix "Service timeout" persistente**: Corregido error que impedía el arranque del Scraper tras priorizar el entorno embebido. Se han añadido las librerías de BigQuery (`google-cloud-bigquery`, `google-auth`) que faltaban en el instalador automático (`start.py`) y se han pre-instalado en el entorno para restaurar el servicio inmediatamente.
